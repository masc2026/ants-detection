...
Detector::Detector(const std::string& modelNameStr) {
    @autoreleasepool {
        NSString* modelName = [NSString stringWithUTF8String:modelNameStr.c_str()];
        
        // Fragt das System dynamisch nach dem Pfad zur kompilierten Modelldatei
        NSURL* modelURL = [[NSBundle mainBundle] URLForResource:modelName withExtension:@"mlmodelc"];
        
        if (!modelURL) {
            NSLog(@"Modell-URL konnte nicht gefunden werden.");
            return;
        }
...
void Detector::exportDetectedFramesUpTo(NSUInteger frameIndexEnde, NSURL* videoInURL, NSURL* videoOutURL) {
    ...
    AVAssetWriter *writer = [[AVAssetWriter alloc] initWithURL:videoOutURL fileType:AVFileTypeMPEG4 error:&error];
    ...
    // Den aktuellen Thread pausieren, bis der Track asynchron geladen wurde
    dispatch_semaphore_wait(trackSemaphore, DISPATCH_TIME_FOREVER);

    if (!videoTrack) {
        NSLog(@"⚠️ Fehler: Konnte Video-Track nicht laden.");
        return;
    }
