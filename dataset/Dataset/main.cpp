...
// Deklaration ggf. in der Header-Datei anpassen!
void setUp(const std::string& arg_clip_kennung, const std::string& arg_video_ordner) {
    ...
    */
    ...
    // Werte aus den Parametern übernehmen
    clip_kennung = arg_clip_kennung;
    set_path = std::string(PROJECT_SOURCE_DIR) + "/data/" + clip_kennung + "/";
    
    // Flexibler Pfad, funktioniert unter macOS und Linux
    fs::path video_base(arg_video_ordner);
    fs::path video_file = video_base / ("clip_" + clip_kennung + ".mp4");
    clip_path = video_file.string();
    ...
    std::cout << "default_config_path: " << default_config_path << endl;
    std::cout << "set_path: " << set_path << endl;
    std::cout << "maske_bw_path: " << maske_bw_path << endl;
    std::cout << "json_path: " << json_path << endl;
    std::cout << "debug_path: " << debug_path << endl;
    std::cout << "yolo_path: " << yolo_path << endl;
    std::cout << "ml_path: " << ml_path << endl;
    std::cout << "clip_path: " << clip_path << endl;
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 3) {
        std::cerr << "\nSyntax: " << argv[0] << " <kennung> <pfad>\n";
        std::cerr << "Beispiel für '/pfad/zu/den/videos/clip_20250415_1646.mp4': " << argv[0] << " 20250415_1646 /pfad/zu/den/videos\n";
        return -1;
    }

    std::string arg_clip_kennung = argv[1];
    std::string arg_video_ordner = argv[2];
    ...
    setUp(arg_clip_kennung, arg_video_ordner);
    ...
    if (maske_bw.empty()) {
        std::cerr << "Fehler beim Laden der Maske!" << std::endl;
        return -1;
    }
